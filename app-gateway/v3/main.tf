...
resource "azurerm_application_gateway" "main" {
  ...
  dynamic "autoscale_configuration" {
    ...
    content {
      min_capacity = lookup(autoscale_configuration.value, "min_capacity", null)
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      ...
      public_ip_address_id          = frontend_ip_configuration.value.private_ip_address == null ? azurerm_public_ip.pip.id : null
      ...
      subnet_id                     = frontend_ip_configuration.value.private_ip_address != null ? data.azurerm_subnet.snet.id : null
    }
  }
  ...
  dynamic "probe" {
    ...
    content {
      ...
      host                                      = probe.value.pick_host_name_from_backend_http_settings ? "" : "127.0.0.1"
  ...
  dynamic "rewrite_rule_set" {
    ...
    content {
      ...
      dynamic "rewrite_rule" {
        for_each = lookup(rewrite_rule_set.value, "rewrite_rules", [])
        content {
...
resource "azurerm_monitor_diagnostic_setting" "pip-diag" {
  ...
  name                       = lower("pip-${local.naming}-${var.additionalcontext}")
...
resource "azurerm_monitor_diagnostic_setting" "agw-diag" {
  count                      = var.log_analytics_workspace_name != null || var.storage_account_name != null ? 1 : 0
  name                       = lower("${local.naming}-diag-${var.additionalcontext}")
