locals {
  ...
  log_category_groups = [
    for group in
    (
      var.log_category_groups != null ?
      var.log_category_groups :
      try(data.azurerm_monitor_diagnostic_categories.main[0].log_category_groups, [])
    ) : group if !contains(var.excluded_log_category_groups, group)
  ]
  ...
  eventhub_authorization_rule_id = coalescelist([for r in var.logs_destinations_ids : split("|", r)[0] if contains(split("/", lower(r)), "authorizationrules")], [null])[0]
  eventhub_name                  = coalescelist([for r in var.logs_destinations_ids : try(split("/", r)[length(split("/", r)) - 1], null) if contains(split("/", lower(r)), "eventhubs")], [null])[0]
