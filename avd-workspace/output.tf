output "resource" {
  ...
  value       = { for k, v in module.avm-res-desktopvirtualization-workspace : k => v.resource }
}

output "resource_id" {
  description = "The ID of the Azure Virtual Desktop application group, keyed by each item in for_each"
  value       = { for k, v in module.avm-res-desktopvirtualization-workspace : k => v.resource_id }
}
