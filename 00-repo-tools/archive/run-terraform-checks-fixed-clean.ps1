...
.EXAMPLE
    .\run-terraform-checks-fixed-clean.ps1 -ModulePath "C:\terraform\modules\storage-account"
    
    Runs standard quality checks on the specified storage account module.

.EXAMPLE
    .\run-terraform-checks-fixed-clean.ps1 -ModulePath ".\modules\aks-cluster" -AzureModuleCheck
    
    Performs quality checks plus Azure compliance validation for an AKS module.

.EXAMPLE
    .\run-terraform-checks-fixed-clean.ps1 -ModulePath "C:\terraform\modules\network" -AzureModuleCheck -SkipValidation
    
    Runs Azure compliance checks while skipping standard structure validation.

.EXAMPLE
    .\run-terraform-checks-fixed-clean.ps1 -Help
    
    Displays comprehensive help information and usage guidance.

.NOTES
    FileName: run-terraform-checks-fixed-clean.ps1
    ...
    • terraform-docs (if not present in WSL)
...
"@ -ForegroundColor Cyan
    ...
    Write-Host "     .\run-terraform-checks-fixed-clean.ps1 -ModulePath `"C:\terraform\modules\storage`"" -ForegroundColor Gray
    
    Write-Host "`n   🔹 Azure Compliance Check:" -ForegroundColor Cyan
    Write-Host "     .\run-terraform-checks-fixed-clean.ps1 -ModulePath `".\modules\aks`" -AzureModuleCheck" -ForegroundColor Gray
    
    Write-Host "`n   🔹 Skip Structure Validation:" -ForegroundColor Cyan
    Write-Host "     .\run-terraform-checks-fixed-clean.ps1 -ModulePath `".\modules\network`" -SkipValidation" -ForegroundColor Gray
    
    Write-Host "`n   🔹 Full Azure Compliance (Skip Standard Validation):" -ForegroundColor Cyan
    Write-Host "     .\run-terraform-checks-fixed-clean.ps1 -ModulePath `"C:\tf\modules\vm`" -AzureModuleCheck -SkipValidation" -ForegroundColor Gray
    ...
    Write-Host "   ✅ Auto-Generated Documentation (terraform-docs)" -ForegroundColor Green
    ...
    Write-Host "Get-Help .\run-terraform-checks-fixed-clean.ps1 -Detailed" -ForegroundColor Gray
    Write-Host "   📋 Examples Only: " -ForegroundColor Cyan -NoNewline
    Write-Host "Get-Help .\run-terraform-checks-fixed-clean.ps1 -Examples" -ForegroundColor Gray
    Write-Host "   🔍 Full Help: " -ForegroundColor Cyan -NoNewline
    Write-Host "Get-Help .\run-terraform-checks-fixed-clean.ps1 -Full" -ForegroundColor Gray
...
function Show-CompletionSummary {
    ...
    Write-Host "`n   2. 📖 Review Generated Documentation:" -ForegroundColor Yellow
    ...
    Write-Host "`n   3. 🔍 Address Any Linting Issues:" -ForegroundColor Yellow
    ...
    Write-Host "`n   4. 🧪 Testing & Validation:" -ForegroundColor Yellow
    ...
    Write-Host "   .\run-terraform-checks-fixed-clean.ps1 -ModulePath `"$ModulePath`" -AzureModuleCheck" -ForegroundColor Gray
    
    Write-Host "`n   📚 Get detailed help:" -ForegroundColor Cyan
    Write-Host "   .\run-terraform-checks-fixed-clean.ps1 -Help" -ForegroundColor Gray
...
Show-CompletionSummary -ModulePath $ModulePath -AzureModuleCheck $AzureModuleCheck -SkipValidation $SkipValidation -ComplianceScore $(if ($AzureModuleCheck) { $complianceScore } else { 0 })
