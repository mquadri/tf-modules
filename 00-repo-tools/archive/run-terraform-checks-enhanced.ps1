# Terraform Module Quality Check Script (Enhanced Version)
# This script runs formatting, linting, and documentation generation for Terraform modules
# 
# Usage: 
#   .\run-terraform-checks-enhanced.ps1 -ModulePath <path-to-module>
#
# Parameters:
...
#   -WslDistribution    : (Optional) Specify a WSL distribution to use (default: auto-detect)
...
function Test-AzureModuleCriterion {
    ...
    # Define requirements as a structured array for better maintainability
    $requirementsList = @(
        @{
    ...
    foreach ($req in $requirementsList) {
        ...
        if ($req.IsDirectory) {
            # Directory check - use single quotes for the path in bash to handle special characters
            ...
            if ($fileCheck -eq "exists") {
                if ($req.Pattern) {
                    # Check for pattern in file - use single quotes around the pattern for bash
...
try {
    ...
    $distributionsInfo = wsl.exe --list --verbose 2>&1 | Out-String
    if ($distributionsInfo -match "no distributions" -or $distributionsInfo -eq "" -or $null -eq $distributionsInfo) {
...
if ([string]::IsNullOrEmpty($WslDistribution)) {
    ...
    if ($distributionsInfo -match "(?:\s|\*)*([^\s]+Ubuntu[^\s]*|Ubuntu[^\s]*|[^\s]*ubuntu[^\s]*)") {
        ...
        $WslDistribution = $WslDistribution -replace '[^\w\-.]', ''
    ...
    # User provided a distribution - validate and clean it
    Write-Host "Using user-specified WSL distribution: $WslDistribution" -ForegroundColor Cyan
    # Clean up any potential special characters or formatting issues
    $WslDistribution = $WslDistribution.Trim() -replace '[^\w\-.]', ''
    if ([string]::IsNullOrEmpty($WslDistribution)) {
...
Write-Host "Distribution parameter length: $($WslDistribution.Length)" -ForegroundColor Gray

# Validate WSL distribution exists and is accessible
Write-Host "Validating WSL distribution '$WslDistribution'..." -ForegroundColor Cyan
try {
    $distroTest = wsl.exe --list --quiet 2>&1
    if ($distroTest -notcontains $WslDistribution) {
        Write-Host "Warning: Distribution '$WslDistribution' not found in WSL list. Attempting to use anyway..." -ForegroundColor Yellow
    }
    
    # Test basic WSL connectivity with the distribution
    Write-Host "Testing WSL connectivity with distribution '$WslDistribution'..." -ForegroundColor Cyan
...
try {
    ...
    # Use wslpath without quotes around distribution name (known issue in WSL)
        ...
        if ($LASTEXITCODE -ne 0 -or $wslPath -eq "") {
            ...
            if ($ModulePath -match "^([A-Za-z]):\\(.*)$") {
                ...
                if ($pathCheck -eq "missing") {
                    Write-Host "Warning: Constructed WSL path does not exist. Check may fail." -ForegroundColor Red
                }
            } else {
    ...
    # Verify the path is accessible in WSL
    $pathAccessCheck = wsl.exe --distribution $WslDistribution --exec bash -c "test -d '$wslPath' && echo 'accessible' || echo 'inaccessible'"
    if ($pathAccessCheck -ne "accessible") {
        Write-Host "Warning: Converted path is not accessible in WSL. Checks may fail." -ForegroundColor Yellow
    }
} catch {
...
try {
    $tflintResult = wsl.exe --distribution $WslDistribution --exec bash -c "command -v tflint > /dev/null 2>&1 || echo 'not-installed'"
    if ($tflintResult -eq "not-installed") {
        Write-Host "tflint is not installed. Installing now..." -ForegroundColor Cyan
        wsl.exe --distribution $WslDistribution --exec bash -c "curl -s https://raw.githubusercontent.com/terraform-linters/tflint/master/install_linux.sh | bash"
        
        # Verify installation was successful
        $tflintVerify = wsl.exe --distribution $WslDistribution --exec bash -c "command -v tflint > /dev/null 2>&1 || echo 'failed'"
    ...
    $output = wsl.exe --distribution $WslDistribution --exec bash -c "cd '$wslPath' && tflint --recursive 2>&1"
    $status = $LASTEXITCODE
...
try {
    ...
    $terraformDocsResult = wsl.exe --distribution $WslDistribution --exec bash -c "command -v terraform-docs > /dev/null 2>&1 || echo 'not-installed'"
    if ($terraformDocsResult -eq "not-installed") {
        ...
        wsl.exe --distribution $WslDistribution --exec bash -c "mkdir -p ~/bin"
        
        # Download and install terraform-docs
        wsl.exe --distribution $WslDistribution --exec bash -c "curl -sSLo ./terraform-docs.tar.gz https://terraform-docs.io/dl/v0.16.0/terraform-docs-v0.16.0-linux-amd64.tar.gz && tar -xzf terraform-docs.tar.gz && chmod +x terraform-docs && mv terraform-docs ~/bin/ && rm -f terraform-docs.tar.gz"
        
        # Add to PATH if not already there
        wsl.exe --distribution $WslDistribution --exec bash -c "grep -q 'export PATH=~/bin:\$PATH' ~/.bashrc || echo 'export PATH=~/bin:\$PATH' >> ~/.bashrc"
        
        # Verify installation was successful
        $verifyResult = wsl.exe --distribution $WslDistribution --exec bash -c "ls ~/bin/terraform-docs > /dev/null 2>&1 || echo 'failed'"
    ...
    # Explicitly adding export PATH to ensure terraform-docs is found
    $output = wsl.exe --distribution $WslDistribution --exec bash -c "cd '$wslPath' && export PATH=~/bin:\$PATH && terraform-docs markdown table . > README_GENERATED.md 2>&1"
    $status = $LASTEXITCODE
    ...
    if ($status -ne 0) {
        ...
        $fileCheck = wsl.exe --distribution $WslDistribution --exec bash -c "test -f '$wslPath/README_GENERATED.md' && echo 'exists' || echo 'missing'"
        if ($fileCheck -eq "missing") {
            ...
            wsl.exe --distribution $WslDistribution --exec bash -c "chmod 664 '$wslPath/README_GENERATED.md'"
            Write-Host "Set read/write permissions on README_GENERATED.md file." -ForegroundColor Cyan
            
            # Verify the file is accessible
            $readableCheck = wsl.exe --distribution $WslDistribution --exec bash -c "test -r '$wslPath/README_GENERATED.md' && echo 'readable' || echo 'not-readable'"
            if ($readableCheck -ne "readable") {
                Write-Host "Warning: README_GENERATED.md might not be readable. Attempting to fix permissions..." -ForegroundColor Yellow
                wsl.exe --distribution $WslDistribution --exec bash -c "sudo chmod 664 '$wslPath/README_GENERATED.md'"
            }
        }
    }
} catch {
...
if (-not $SkipValidation) {
    ...
    try {
        ...
        foreach ($file in $expectedFiles) {
            $fileCheck = wsl.exe --distribution $WslDistribution --exec bash -c "test -e '$wslPath/$file' && echo 'exists' || echo 'missing'"
            if ($fileCheck -eq "missing") {
...
if ($AzureModuleCheck) {
    $complianceScore = Test-AzureModuleCriterion -ModulePath $ModulePath -WslPath $wslPath -Distribution $WslDistribution
