# Terraform Module Quality Check Script (Enhanced Version 2)
# This script runs formatting, linting, and documentation generation for Terraform modules
# 
# Usage: 
#   .\run-terraform-checks-enhanced-v2.ps1 -ModulePath <path-to-module>
#
# Parameters:
...
# Function to safely execute WSL commands with proper error handling
function Invoke-WslCommand {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "Warning: $Description failed with exit code $LASTEXITCODE" -ForegroundColor Yellow
...
# Function to get available WSL distributions with improved parsing
function Get-WslDistributions {
    try {
        Write-Host "Detecting available WSL distributions..." -ForegroundColor Cyan
        
        # Get distribution list in parseable format
        ...
        # Parse distributions and clean up encoding issues
        $distributions = @()
        $rawOutput -split "`n" | ForEach-Object {
            $distro = $_.Trim() -replace '\x00', '' -replace '[^\w\-.]', ''
        ...
        if ($distributions.Count -eq 0) {
            Write-Host "Warning: No WSL distributions found in output: $rawOutput" -ForegroundColor Yellow
        } else {
...
# Function to select the best WSL distribution
function Select-WslDistribution {
    ...
    if (![string]::IsNullOrEmpty($PreferredDistribution)) {
        $cleanDistro = $PreferredDistribution.Trim() -replace '[^\w\-.]', ''
    ...
    # Priority: Ubuntu distributions > Default distribution > First available
    
    # Look for Ubuntu distributions first
    $ubuntuDistros = $AvailableDistributions | Where-Object { $_ -match "Ubuntu" -or $_ -match "ubuntu" }
    if ($ubuntuDistros) {
        Write-Host "Selected Ubuntu distribution: $($ubuntuDistros[0])" -ForegroundColor Green
...
if (-not $testResult.Success) {
    Write-Host "Error: Selected WSL distribution '$selectedDistribution' is not accessible. Output: $($testResult.Output)" -ForegroundColor Red
    exit 1
}
...
if ($pathConversionResult.Success) {
    ...
    if ($ModulePath -match "^([A-Za-z]):\\(.*)$") {
        ...
        Write-Host "Manual path conversion: $ModulePath -> $wslPath" -ForegroundColor Yellow
          # Verify path exists
        $pathCheckResult = Invoke-WslCommand -Distribution $selectedDistribution -Command "test -d '$wslPath'; if [ \$? -eq 0 ]; then echo 'exists'; else echo 'missing'; fi" -Description "Path existence check"
...
$terraformCheckResult = Invoke-WslCommand -Distribution $selectedDistribution -Command "if command -v terraform > /dev/null 2>&1; then echo 'installed'; else echo 'not-installed'; fi" -Description "Terraform installation check"
...
if ($terraformCheckResult.Success -and $terraformCheckResult.Output.Trim() -eq "installed") {
    ...
    $terraformInstallResult = Invoke-WslCommand -Distribution $selectedDistribution -Command "curl -fsSL https://apt.releases.hashicorp.com/gpg | sudo apt-key add -; sudo apt-add-repository 'deb [arch=amd64] https://apt.releases.hashicorp.com \$(lsb_release -cs) main'; sudo apt-get update; sudo apt-get install -y terraform" -Description "Terraform installation"
    
    if (-not $terraformInstallResult.Success) {
        Write-Host "Error: Failed to install Terraform. Output: $($terraformInstallResult.Output)" -ForegroundColor Red
        exit 1
    }
      # Verify installation
    $terraformVerifyResult = Invoke-WslCommand -Distribution $selectedDistribution -Command "if command -v terraform > /dev/null 2>&1; then echo 'verified'; else echo 'failed'; fi" -Description "Terraform installation verification"
