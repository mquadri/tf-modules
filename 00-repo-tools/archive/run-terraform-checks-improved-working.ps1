...
function Test-AzureModuleCriterion {
    ...
    foreach ($req in $requirementsList) {
        ...
        if ($req.IsDirectory) {
            ...
            $fileCheck = wsl.exe --distribution "$Distribution" --exec bash -c $fileCommand            if ($fileCheck -eq "exists") {
                if ($req.Pattern) {
    ...
    if ($percentCompliance -ge 80) {
        Write-Host "`n$complianceMessage" -ForegroundColor Green
    } elseif ($percentCompliance -ge 60) {
        Write-Host "`n$complianceMessage" -ForegroundColor Yellow
    } else {
        Write-Host "`n$complianceMessage" -ForegroundColor Red
    }
...
try {
    ...
    if ($ModulePath -match '^([A-Za-z]):\\(.*)$') {
        ...
        $pathTestCmd = "test -d '$wslPath' $andOperator echo 'exists' $orOperator echo 'missing'"
        $pathTest = wsl.exe --distribution $ubuntuDistro --exec bash -c $pathTestCmd 2>&1
...
try {
    ...
    if ($terraformDocsResult -eq "not-installed") {
        ...
        wsl.exe --distribution $ubuntuDistro --exec bash -c "curl -sSLo ./terraform-docs.tar.gz https://terraform-docs.io/dl/v0.16.0/terraform-docs-v0.16.0-linux-amd64.tar.gz $andOperator tar -xzf terraform-docs.tar.gz $andOperator chmod +x terraform-docs $andOperator mv terraform-docs ~/bin/ $andOperator rm -f terraform-docs.tar.gz"        # Add to PATH if not already there
        $pathUpdateCmd = "grep -q 'export PATH=~/bin:\`$PATH' ~/.bashrc $orOperator echo 'export PATH=~/bin:\`$PATH' | tee -a ~/.bashrc"
        wsl.exe --distribution $ubuntuDistro --exec bash -c $pathUpdateCmd
        
        # Verify installation was successful
        $verifyCmd = "ls ~/bin/terraform-docs $orOperator echo 'failed'"
        $verifyResult = wsl.exe --distribution $ubuntuDistro --exec bash -c $verifyCmd
...
if (-not $SkipSuperLinter) {
    ...
    try {        # Check if Docker is available
        $dockerCheckCmd = "command -v docker $orOperator echo 'not-installed'"
        $dockerCheck = wsl.exe --distribution $ubuntuDistro --exec bash -c $dockerCheckCmd
        if ($dockerCheck -eq "not-installed") {
            Write-Host "Docker is not installed in WSL. Installing Docker..." -ForegroundColor Cyan
            # Install Docker in WSL
            Write-Host "Installing Docker in WSL..." -ForegroundColor Cyan            wsl.exe --distribution $ubuntuDistro --exec bash -c "sudo apt-get update"
            ...
            $dockerVerifyCmd = "command -v docker $orOperator echo 'failed'"
            $dockerVerify = wsl.exe --distribution $ubuntuDistro --exec bash -c $dockerVerifyCmd
        ...
        $readmeCheckCmd = "test -f '$wslPath/README.md' $andOperator echo 'exists' $orOperator echo 'missing'"
        $readmeCheck = wsl.exe --distribution $ubuntuDistro --exec bash -c $readmeCheckCmd
        if ($readmeCheck -eq "missing") {
            Write-Host "Warning: README.md not found in module directory. Super-Linter analysis skipped." -ForegroundColor Yellow
        } else {
            Write-Host "Running Super-Linter for README.md analysis..." -ForegroundColor Cyan
              # Run Super-Linter with focus on markdown files
            $superlinterCmd = "cd '$wslPath' $andOperator docker run --rm -e RUN_LOCAL=true -e USE_FIND_ALGORITHM=true -e VALIDATE_MARKDOWN=true -e VALIDATE_ALL_CODEBASE=false -v `$(pwd):/tmp/lint github/super-linter:latest"
            $superlinterOutput = wsl.exe --distribution $ubuntuDistro --exec bash -c $superlinterCmd 2>&1
            $superlinterExitCode = $LASTEXITCODE
            
            if ($superlinterExitCode -eq 0) {
                Write-Host "Super-Linter analysis completed successfully." -ForegroundColor Green
            } else {
                Write-Host "Super-Linter found issues in README.md:" -ForegroundColor Yellow
                Write-Host $superlinterOutput -ForegroundColor Gray
            }
        }
    } catch {
...
if (-not $SkipValidation) {
    ...
    try {
        ...
        foreach ($file in $expectedFiles) {
            $fileCheckCmd = "test -e '$wslPath/$file' $andOperator echo 'exists' $orOperator echo 'missing'"
            $fileCheck = wsl.exe --distribution $ubuntuDistro --exec bash -c $fileCheckCmd
            if ($fileCheck -eq "missing") {
...
🚀 USAGE TIPS:
   • Run with -AzureModuleCheck for marketplace readiness: 
     .\run-terraform-checks-improved-working.ps1 -ModulePath "$ModulePath" -AzureModuleCheck
   
   • Get detailed help anytime:
     .\run-terraform-checks-improved-working.ps1 -Help
   
   • Standard PowerShell help:
     Get-Help .\run-terraform-checks-improved-working.ps1 -Detailed
