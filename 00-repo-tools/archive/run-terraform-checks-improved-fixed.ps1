...
if (-not $SkipSuperLinter) {
    ...
    try {
        ...
        if ($readmeCheck -eq "missing") {
            ...
            Write-Host "README.md found. Running Super-Linter analysis..." -ForegroundColor Cyan
            
            # Initialize temporary Git repository for Super-Linter (it requires Git)
            $gitInitCommand = "cd '$wslPath' && if [ ! -d .git ]; then git init >/dev/null 2>&1 && git config user.email 'terraform-check@local.dev' && git config user.name 'Terraform Check' && git add . >/dev/null 2>&1 && git commit -m 'temp commit for linting' >/dev/null 2>&1; fi"
            wsl.exe --distribution $ubuntuDistro --exec bash -c $gitInitCommand
            
            # Run Super-Linter for markdown validation
            Write-Host "Analyzing README.md with Super-Linter..." -ForegroundColor Cyan
            $superlinterCommand = "cd '$wslPath' && sudo docker run --rm -e RUN_LOCAL=true -e VALIDATE_MARKDOWN=true -e LOG_LEVEL=INFO -v `"`$(pwd):/tmp/lint`" --workdir /tmp/lint github/super-linter:latest"
            $superlinterOutput = wsl.exe --distribution $ubuntuDistro --exec bash -c $superlinterCommand
            ...
            if ($superlinterOutput) {
                ...
                if ($superlinterOutput -match "FATAL.*Exiting with errors found" -or 
                    ...
                    foreach ($line in $outputLines) {
                        if ([string]::IsNullOrWhiteSpace($line) -or $line.Length -lt 10) { 
                            continue 
                        }
                        ...
                        elseif ($line -match "README\.md.*\[ERROR\]") {
                            $markdownErrors += $line
                        }
                        elseif ($line -match "README\.md" -and $line -match "(line|:\d+|ERROR|FAIL)") {
                            $markdownErrors += $line
                        }
                    }
