variable "kind" {
  type        = string
  description = "(Required) Specifies the type of Cognitive Service Account that should be created. Possible values are `CustomVision.Prediction` and `CustomVision.Training`. Changing this forces a new resource to be created."
  nullable    = false
  validation {
    condition     = (contains(["CustomVision.Prediction", "CustomVision.Training"], var.kind))
    error_message = "Cognitive Service Account kind can only be \"CustomVision.Prediction\" or \"CustomVision.Training\"."
  }
}
...
variable "resource_group_name" {
  description = "Resource group name for where openAI Service will be deployed"
  type        = string
}

# cognitive service settings

variable "name" {
  type        = string
  default     = null
  description = "allow override of the Cognitive Service Account name"
}
