...
resource "azurerm_web_application_firewall_policy" "policy" {
  for_each = { for waf in var.waf_policies : waf.policy_name => waf }
  ...
  policy_settings {
    ...
    file_upload_limit_in_mb     = each.value.policy_file_limit
    request_body_check          = each.value.policy_request_body_check_enabled
    max_request_body_size_in_kb = each.value.policy_max_body_size
    # This is a parameter that is not supported by AZRM < 4.0 which is most of the cases 
    # request_body_enforcement                  = each.value.policy_request_body_enforcement
    request_body_inspect_limit_in_kb          = each.value.policy_request_body_inspect_limit_in_kb
    js_challenge_cookie_expiration_in_minutes = each.value.policy_js_challenge_cookie_expiration_in_minutes
  }

  dynamic "custom_rules" {
    for_each = each.value.custom_rules_configuration
  ...
  managed_rules {
    dynamic "managed_rule_set" {
      for_each = each.value.managed_rule_set_configuration
    ...
    dynamic "exclusion" {
      for_each = each.value.exclusion_configuration
